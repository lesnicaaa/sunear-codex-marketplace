...
The main workflow waits for the customer's PDF, creates the real project and verified designs, audits every designed item against its source, publishes the project's professional interpretation, then completes the market-backed quotation and returns it as a downloadable MCP resource. Professional interpretation separates facts, deductions, commercial inference and potential requirements; only the user accepts, rejects or defers those requirements. Existing-project operations run only when explicitly requested. The trusted session hook downloads the checksum-pinned receiver `0.9.0` for the current platform and installs a user login service: a macOS LaunchAgent or a Windows interactive scheduled task. The existing receiver continues after conversations end and starts on login. It receives scoped WebSocket wakeups and reconciles missed notifications every 60 seconds; the server still owns task authorization, claims and fenced settlement. Background login never opens authorization pages; explicit setup repairs missing authorization. This prerelease rebuilds both supported artifacts with verified Bun 1.4.1 and compiled control-socket/WebSocket gates. Local development, this Mac against Stage, and Mac Studio against Stage require separate acceptance. No project checkout, Node.js, Homebrew or separate receiver installation is required.
