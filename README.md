...
The main workflow waits for the customer's PDF, creates the real project and verified Engine designs, performs a separate source-versus-Engine audit for every designed item, publishes the project's professional interpretation, then completes the market-backed quotation and returns it as a downloadable MCP resource. Professional interpretation keeps sourced project facts, expert deductions, commercial inference, and potential requirements separate; only the user may accept, reject, or defer those requirements. Accessing the newest existing project, re-exporting a document, or another focused operation remains a separate shortcut and runs only when explicitly requested. The session hook downloads the already-published checksum-pinned receiver `0.7.3` for the current platform on first use and starts it automatically; this plugin-only release does not upload the compiler runtime again. No project checkout, Node.js, Homebrew, or separate receiver command is required.
