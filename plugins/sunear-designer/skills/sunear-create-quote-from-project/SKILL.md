...
1. Complete the `sunear.agent-connection/7` preflight with host-managed OAuth and authenticated `workflow_context` / `get_personal_agent_connection` calls. The user initiates work in their own Codex session. OAuth does not grant personal autonomy: obtain explicit user consent before enabling automatic design/quote confirmation, translation and export. Do not install a background runtime or run non-business diagnostics. Codex Desktop uses its host-managed authentication surface, never task-shell `codex mcp login`; CLI uses its native host entry. Open the official authorization URL in the connected user Chrome profile or the default daily browser. Never import cookies or extract credentials. Report credential-persistence failure separately from user denial. Use weaker file credential storage only after explicit consent.
2. Call the authenticated Sunear MCP tool `workflow_context` with one client-stable `agentWorkId` before any project mutation. Preserve the returned `agentWorkBindingId` and include it in every later business-workflow tool call for this work. Reusing the same `agentWorkId` resumes the contract revision already bound to this work; do not replace it merely because a newer revision is published. Follow the returned connection workflow, business workflow, commercial contract, schemas, tool order, and completion evidence.
